...
async function allocateItems() {
  try {
    ...
    await Promise.all(props.allocation.map(async (stock: Record<string, string>) => await fetchy(`/api/inventory/allocate/${stock._id}`, "PATCH", { body: { update: { count: stock.allocation } } })));
  } catch {
    return;
  }
  emit("close");
